...
namespace C3Mod
{
	...
	internal static class GetDataHandlers
	{
		...
		private static bool HandlePlayerKillMe(GetDataHandlerArgs args)
		{
			...
			short Damage = args.Data.ReadInt16();
			...
			if (player.GameType == "ffa")
			{
				...
				C3Tools.BroadcastMessageToGametype("ffa",
					player.KillingPlayer.PlayerName + " - Score : " + player.KillingPlayer.FFAScore + " -- kills -- " +
					player.PlayerName + " - Score : " + player.FFAScore, Color.Black);
				player.Dead = true;
		...
		{
			int index = args.Player.Index; //Attacking Player
			var PlayerID = (byte) args.Data.ReadByte(); //Damaged Player
			var hitDirection = (byte) args.Data.ReadByte();
			short Damage = args.Data.ReadInt16();
			var player = C3Tools.GetC3PlayerByIndex(PlayerID);
			bool PVP = args.Data.ReadBoolean();
