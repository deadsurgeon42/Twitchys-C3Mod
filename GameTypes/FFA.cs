...
namespace C3Mod.GameTypes
{
	internal class FFA
	{
		...
		public static bool FFARunning;
		public static bool FFACountdown;
		public static Vector2 FFASpawn;
		...
		public static void OnUpdate(EventArgs args)
		{
			lock (C3Mod.C3Players)
			{
				if (C3Mod.VoteRunning && (C3Mod.VoteType == "ffa"))
				{
					...
					{
						C3Tools.BroadcastMessageToGametype("ffa", "Vote to play Free For All passed, Teleporting to start position",
							Color.DarkCyan);
					...
					if ((tick > C3Mod.C3Config.VoteNotifyInterval * 1000) && (VoteCount > 0))
					{
						if ((VoteCount != 1) && (VoteCount < C3Mod.C3Config.VoteTime / C3Mod.C3Config.VoteNotifyInterval))
						{
							...
							C3Tools.BroadcastMessageToGametype("", "Vote to play Free For All in progress, type /join to join the lobby",
								Color.Cyan);
						}
