...
namespace C3Mod.GameTypes
{
	...
	internal class Duel
	{
		...
		public static bool DuelRunning;
		public static bool DuelCountdown;
		...
		public static int Team1PlayerScore;
		public static int Team2PlayerScore;
		...
		public static void OnUpdate(EventArgs args)
		{
				...
				foreach (var player in C3Mod.C3Players)
					if (player.Challenging != null)
					{
						...
						if (tick > C3Mod.C3Config.DuelNotifyInterval * 1000)
						{
							...
							{
								player.Challenging.SendMessage(
									"You have been challenged to a duel by: " + player.PlayerName + ", Arena: " +
									Arenas[player.Challenging.ChallengeArena - 1].Name, Color.Cyan);
						...
						else if (player.ChallengeNotifyCount == 0)
						{
							...
							player.SendMessage(
								player.Challenging.PlayerName + ": Did not answer your challenge in the required amount of time", Color.DarkCyan);
			...
			if (DuelCountdown)
			{
				...
				if ((tick > 1000) && (StartCount > -1))
					if (TpToSpawnPoint() > 0)
					{
						...
						{
							C3Tools.BroadcastMessageToGametype("1v1", "Game starting in " + StartCount + "...", Color.Cyan);
			...
			if (DuelRunning)
			{
							...
							if (Main.player[player.Index].dead)
								if (player.Team == 3)
								{
									...
									if (Team2PlayerScore != C3Mod.C3Config.DuelScoreLimit)
									{
										...
										foreach (var players in C3Mod.C3Players)
				...
				if (Team2PlayerScore == C3Mod.C3Config.DuelScoreLimit)
				{
					...
					C3Tools.BroadcastMessageToGametype("", Team2Player.PlayerName + ": Beat " + Team1Player.PlayerName + " in a duel!",
						Color.DarkCyan);
				...
				if (Team1PlayerScore == C3Mod.C3Config.DuelScoreLimit)
				{
					...
					C3Tools.BroadcastMessageToGametype("", Team1Player.PlayerName + ": Beat " + Team2Player.PlayerName + " in a duel!",
						Color.DarkCyan);
		...
		public static int TpToSpawnPoint()
		{
			try
			{
				var Team1Player = false;
				var Team2Player = false;
				...
				for (var i = 0; i < C3Mod.C3Players.Count; i++)
				{
					...
					if (C3Mod.C3Players[i].Team == 3)
					{
						...
						if ((C3Mod.C3Players[i].TSPlayer.X / 16 != DuelSpawns[0].X) ||
						    (C3Mod.C3Players[i].TSPlayer.Y / 16 != DuelSpawns[0].Y))
					...
					else if (C3Mod.C3Players[i].Team == 4)
					{
						Team2Player = true;
						C3Mod.C3Players[i].TSPlayer.TpLock = false;
						if ((C3Mod.C3Players[i].TSPlayer.X / 16 != DuelSpawns[1].X) ||
						    (C3Mod.C3Players[i].TSPlayer.Y / 16 != DuelSpawns[1].Y))
