...
namespace C3Mod
{
	...
	public class C3Mod : TerrariaPlugin
	{
		...
		internal void OnInitialize(EventArgs args)
		{
			...
			var setflags = false;
			var setspawns = false;
			var managec3settings = false;
			var cvote = false;
			var duel = false;
			foreach (var group in TShock.Groups.groups)
			...
			switch (C3Config.TeamColor1)
			{
				...
				case 4:
					...
					break;
				case 5:
					Commands.ChatCommands.Add(new Command("c3mod.setflags", C3Commands.SetCTFTeam1Flag, "setctfpinkflag"));
					Commands.ChatCommands.Add(new Command("c3mod.setflags", C3Commands.SetCTFTeam1Spawn, "setctfpinkspawn"));
					Commands.ChatCommands.Add(new Command("c3mod.setflags", C3Commands.SetOneFlagTeam1Spawn, "setoneflagpinkspawn"));
					Commands.ChatCommands.Add(new Command("c3mod.setspawns", C3Commands.SetDuelTeam1Spawn, "setduelpinkspawn"));
					Commands.ChatCommands.Add(new Command("c3mod.setspawns", C3Commands.SetTDMTeam1Spawn, "settdmpinkspawn"));
					break;
			}
			switch (C3Config.TeamColor2)
			{
				case 1:
				...
				case 4:
					...
					break;
				case 5:
					Commands.ChatCommands.Add(new Command("c3mod.setflags", C3Commands.SetCTFTeam2Flag, "setctfpinkflag"));
					Commands.ChatCommands.Add(new Command("c3mod.setflags", C3Commands.SetCTFTeam2Spawn, "setctfpinkspawn"));
					Commands.ChatCommands.Add(new Command("c3mod.setflags", C3Commands.SetOneFlagTeam2Spawn, "setoneflagpinkspawn"));
					Commands.ChatCommands.Add(new Command("c3mod.setspawns", C3Commands.SetDuelTeam2Spawn, "setduelpinkspawn"));
					Commands.ChatCommands.Add(new Command("c3mod.setspawns", C3Commands.SetTDMTeam2Spawn, "settdmpinkspawn"));
					break;
			}
			...
			Commands.ChatCommands.Add(new Command("c3mod.vote", C3Commands.StartVote, "vote"));
			Commands.ChatCommands.Add(new Command("c3mod.joinvote", C3Commands.JoinVote, "join"));
			Commands.ChatCommands.Add(new Command("c3mod.managec3settings", C3Commands.Stop, "stop"));
			Commands.ChatCommands.Add(new Command(C3Commands.Quit, "c3quit"));
			...
			if ((ArenaCount = SQLEditor.ReadColumn("FlagPoints", "Name", new List<SqlValue>()).Count) != 0)
				for (var i = 0; i < ArenaCount; i++)
				{
					...
					int redx = int.Parse(SQLEditor.ReadColumn("FlagPoints", "RedX", new List<SqlValue>())[i].ToString());
					int redy = int.Parse(SQLEditor.ReadColumn("FlagPoints", "RedY", new List<SqlValue>())[i].ToString());
					int bluex = int.Parse(SQLEditor.ReadColumn("FlagPoints", "BlueX", new List<SqlValue>())[i].ToString());
					int bluey = int.Parse(SQLEditor.ReadColumn("FlagPoints", "BlueY", new List<SqlValue>())[i].ToString());
					int redspawnx = int.Parse(SQLEditor.ReadColumn("FlagPoints", "RedSpawnX", new List<SqlValue>())[i].ToString());
					int redspawny = int.Parse(SQLEditor.ReadColumn("FlagPoints", "RedSpawnY", new List<SqlValue>())[i].ToString());
					int bluespawnx = int.Parse(SQLEditor.ReadColumn("FlagPoints", "BlueSpawnX", new List<SqlValue>())[i].ToString());
					int bluespawny = int.Parse(SQLEditor.ReadColumn("FlagPoints", "BlueSpawnY", new List<SqlValue>())[i].ToString());

					CTF.Arenas.Add(new CTFArena(new Vector2(redx, redy), new Vector2(bluex, bluey), new Vector2(redspawnx, redspawny),
						new Vector2(bluespawnx, bluespawny), name));
				}

			#endregion FlagPoints
			...
			if ((ArenaCount = SQLEditor.ReadColumn("DuelSpawns", "Name", new List<SqlValue>()).Count) != 0)
				for (var i = 0; i < ArenaCount; i++)
				{
					...
					int redx = int.Parse(SQLEditor.ReadColumn("DuelSpawns", "RedSpawnX", new List<SqlValue>())[0].ToString());
					int redy = int.Parse(SQLEditor.ReadColumn("DuelSpawns", "RedSpawnY", new List<SqlValue>())[0].ToString());
					int bluex = int.Parse(SQLEditor.ReadColumn("DuelSpawns", "BlueSpawnX", new List<SqlValue>())[0].ToString());
					int bluey = int.Parse(SQLEditor.ReadColumn("DuelSpawns", "BlueSpawnY", new List<SqlValue>())[0].ToString());

					Duel.Arenas.Add(new DuelArena(new Vector2(redx, redy), new Vector2(bluex, bluey), name));
				}

			#endregion DuelSpawns
			...
			if ((ArenaCount = SQLEditor.ReadColumn("OneFlagPoints", "Name", new List<SqlValue>()).Count) != 0)
				for (var i = 0; i < ArenaCount; i++)
				{
					...
					int flagx = int.Parse(SQLEditor.ReadColumn("OneFlagPoints", "FlagX", new List<SqlValue>())[i].ToString());
					int flagy = int.Parse(SQLEditor.ReadColumn("OneFlagPoints", "FlagY", new List<SqlValue>())[i].ToString());
					int redspawnx = int.Parse(SQLEditor.ReadColumn("OneFlagPoints", "RedSpawnX", new List<SqlValue>())[i].ToString());
					int redspawny = int.Parse(SQLEditor.ReadColumn("OneFlagPoints", "RedSpawnY", new List<SqlValue>())[i].ToString());
					int bluespawnx = int.Parse(SQLEditor.ReadColumn("OneFlagPoints", "BlueSpawnX", new List<SqlValue>())[i].ToString());
					int bluespawny = int.Parse(SQLEditor.ReadColumn("OneFlagPoints", "BlueSpawnY", new List<SqlValue>())[i].ToString());

					OneFlagCTF.Arenas.Add(new OneFlagArena(new Vector2(flagx, flagy), new Vector2(redspawnx, redspawny),
						new Vector2(bluespawnx, bluespawny), name));
				}

			#endregion OneFlagPoints
			...
			if ((ArenaCount = SQLEditor.ReadColumn("TDMSpawns", "Name", new List<SqlValue>()).Count) != 0)
				for (var i = 0; i < ArenaCount; i++)
				{
					...
					int redx = int.Parse(SQLEditor.ReadColumn("TDMSpawns", "RedSpawnX", new List<SqlValue>())[i].ToString());
					int redy = int.Parse(SQLEditor.ReadColumn("TDMSpawns", "RedSpawnY", new List<SqlValue>())[i].ToString());
					int bluex = int.Parse(SQLEditor.ReadColumn("TDMSpawns", "BlueSpawnX", new List<SqlValue>())[i].ToString());
					int bluey = int.Parse(SQLEditor.ReadColumn("TDMSpawns", "BlueSpawnY", new List<SqlValue>())[i].ToString());
			...
			if ((ArenaCount = SQLEditor.ReadColumn("FFASpawns", "Name", new List<SqlValue>()).Count) != 0)
				for (var i = 0; i < ArenaCount; i++)
				{
					...
					int x = int.Parse(SQLEditor.ReadColumn("FFASpawns", "SpawnX", new List<SqlValue>())[i].ToString());
					int y = int.Parse(SQLEditor.ReadColumn("FFASpawns", "SpawnY", new List<SqlValue>())[i].ToString());
			...
			{
				Apocalypse.PlayerSpawn.X =
					int.Parse(SQLEditor.ReadColumn("Apocalypse", "SpawnX", new List<SqlValue>())[0].ToString());
				Apocalypse.PlayerSpawn.Y =
					int.Parse(SQLEditor.ReadColumn("Apocalypse", "SpawnY", new List<SqlValue>())[0].ToString());
				Apocalypse.MonsterSpawn.X =
					int.Parse(SQLEditor.ReadColumn("Apocalypse", "MonsterSpawnX", new List<SqlValue>())[0].ToString());
				Apocalypse.MonsterSpawn.Y =
					int.Parse(SQLEditor.ReadColumn("Apocalypse", "MonsterSpawnY", new List<SqlValue>())[0].ToString());
				Apocalypse.SpectatorArea.X =
					int.Parse(SQLEditor.ReadColumn("Apocalypse", "SpectatorSpawnX", new List<SqlValue>())[0].ToString());
				Apocalypse.SpectatorArea.Y =
					int.Parse(SQLEditor.ReadColumn("Apocalypse", "SpectatorSpawnY", new List<SqlValue>())[0].ToString());
			}
		...
		internal void OnGreetPlayer(GreetPlayerEventArgs args)
		{
			...
			if (C3Config.ShowWelcomeMessage)
				TShock.Players[args.Who].SendMessage("This server is running C3Mod, created by Twitchy. C3Mod is now open source.",
					Color.Cyan);
		}
		...
		internal void OnUpdate(EventArgs args)
		{
			if (C3Config.C3TeamsLocked)
				lock (C3Players)
				{
					foreach (var player in C3Players)
						if (player.GameType == "")
		...
		internal void OnLeave(LeaveEventArgs args)
		{
			lock (C3Players)
			{
				for (var i = 0; i < C3Players.Count; i++)
					if (C3Players[i].Index == args.Who)
					{
						C3Players.RemoveAt(i);
						break;
					}
			}
		}
